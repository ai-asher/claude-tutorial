...
| `/cd` | - | 切换会话工作目录，不破坏 prompt 缓存 | `/cd ../another-project` |
| `/goal` | - | 设定完成条件，Claude 跨轮次自动工作直到达成 | `/goal 所有测试通过` |
...
| `/workflows` | - | 查看动态工作流的运行记录与进度 | `/workflows` |
| `/schedule` | - | 创建定时任务，按 cron 表达式运行 | `/schedule` |
| `/tasks` | - | 查看后台任务和定时任务的状态 | `/tasks` |
| `/reload-skills` | - | 重新扫描技能目录，无需重启会话 | `/reload-skills` |
| `/scroll-speed` | - | 调节鼠标滚轮速度，带实时预览 | `/scroll-speed` |
| `/config` | - | 查看 / 修改设置，支持 `/config key=value` 直接赋值 | `/config thinking=false` |
...
| `/goal` | 2.1.16x | 设定完成条件，跨轮次自动工作 |
| `/workflows` | 2.1.16x | 查看动态工作流运行记录 |
| `/cd` | 2.1.169 | 不破坏缓存切换工作目录 |
| `/reload-skills` | 2.1.16x | 重新扫描技能目录 |
| `/scroll-speed` | 2.1.16x | 调节滚轮速度 |
| `/config key=value` | 2.1.181 | 直接在 prompt 里赋值任意设置 |
