...
### /reload-skills — 改完即时生效

开发 Skill 时反复重启会话很烦。从新版本起，改完 `SKILL.md` 后直接：

```bash
/reload-skills
```

会重新扫描技能目录，无需重启会话即可加载最新改动（只重新加载有变化的 skill）。

### claude plugin init — 一键脚手架

不想从空文件开始，可以让 Claude Code 帮你生成插件骨架：

```bash
claude plugin init my-plugin
```

会在 `.claude/skills` 下脚手架一个新插件的目录结构。

### 嵌套 .claude/skills

子目录里的 `.claude/skills` 现在也能加载——当你在该目录下工作时自动生效。如果和上层有同名 skill，嵌套的那个会以 `<目录>:<名字>` 形式出现，两者都可用，不会互相覆盖。

::: tip 就近优先
嵌套 `.claude/` 目录里，离当前工作目录最近的 agent / workflow / output-style 在同名冲突时获胜。
:::
