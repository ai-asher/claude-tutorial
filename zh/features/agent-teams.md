...
Agent Teams 目前是实验性功能，需要通过环境变量开启：
...
::: tip 2.1.178 起：无需手动建团队
早期版本需要先用 `TeamCreate` 工具显式创建团队。**从 v2.1.178 起，`TeamCreate` 和 `TeamDelete` 工具已移除**——开启实验开关后，每个会话自动拥有一个隐式团队。你直接用 Agent 工具的 `name` 参数 spawn 队友即可，不需要任何建队步骤。Agent 工具上的 `team_name` 参数仍被接受但会被忽略。

此外，**子 agent 现在也能 spawn 自己的子 agent**（最多嵌套 5 层），适合更深的任务分解。
:::
...
```mermaid
graph LR
    TC["隐式团队<br/>自动就绪"] --> TKC["TaskCreate<br/>创建子任务"]
...
开启实验开关后，会话自动拥有一个隐式团队，Team Lead（主 Agent）直接用 Agent 工具的 `name` 参数 spawn 队友即可。（早期版本的 `TeamCreate` 工具已在 v2.1.178 移除。）
...
上一篇：[MCP Servers 外部工具集成 <-](/zh/features/mcp-servers) | 下一篇：[动态工作流 ->](/zh/features/workflows)
